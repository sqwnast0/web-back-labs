...
def register():
    ...
    if password != confirm:
        return render_template('lab8/register.html', error='Пароли не совпадают')
    ...
    if cur.fetchone():
        db_close(conn, cur)
        return render_template('lab8/register.html', error='Пользователь уже существует')
    ...
    db_close(conn, cur)
    return render_template('lab8/success_registration.html', login=login)
